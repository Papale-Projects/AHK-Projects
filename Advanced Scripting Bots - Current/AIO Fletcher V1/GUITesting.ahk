...
Gui, Add, Text, cYellow x40 y55 w900 h20 vFletchCount1 +BackgroundTrans,Bows Fletched:
...
LoggedOutFor()
{
	If (counter >= 28800)
	{
		...
		{
			Logout()
			MsgBox, "Time has reached antiban hardstop! You have reached the daily limit. 8H above"
			Exitapp
		}
	}
	If (counter >= 21600 && counter < 28800)
	{
		...
		{
			Logout()
			MsgBox, "Time has reached antiban hardstop! You have reached the daily limit. 8H"
			Exitapp
		}
	}
...
WinGetPos, GuiX, GuiY,GuiW,GuiH, ahk_class SunAwtFrame
upY := GuiH-150
Gui, Show, x0 y650 w450 h143 NoActivate
return