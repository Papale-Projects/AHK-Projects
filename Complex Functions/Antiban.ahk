...
;My left-click function with added randomized sleeps and the chance to double click
;Variables to specify click location for mouse movement after click and toggle double clicking chance
...
;My right-clicking function with added randomized sleeps
...
;Functional, but dated, needs to be updated with the GDI+ class functions
;Randomly right-clicks on targets to mimic my ticks
;Recode to add variable for target color and variation
RandomRightClicker()
{
	Random, picker, 1, 3
	If (Picker = 1)
...
;Randomizer for any antiban functions
;Notation out conflicting antiban functions
AntibanAny()
{
	Random, Anti, 1, 2016
	If (Anti = 1)
...
;Randomizer for actions that can be prefored while inside the banking window
AntibanBankwindow()
{
	Random, Anti, 1, 21
	If (Anti = 1)
...
;Randomizer for the logout functions
AntiBanLog()
{
	...
	{
		Logout()
		;Replace current function and test
		LogoutTimer()
		LogoutIn()
	}
}

;Randomizer for the ingame move function
AntibanClicks()
{
	Random, Anti3, 1, 6
	If (Anti3 = 1)
...
;Randomizer for the minimap move function
AntibanMinimap()
{
	Random, Anti3, 1, 3
	If (Anti3 = 1)
...
;Functional, but dated code
;Randomly rotates the camera ingame to simulate my on ticks
CameraSpinArrows()
{
...
;Functional and updated code
;Sets the window focus to another window, moves the mouse off of the game window, and then sleeps
;Mimic someone watching a video or navigating the internet
OffScreenMouse()
{
	AntibanOffS2++
	AntibanOffS()
...
;A Longer version of the offscreen mousemovement above
;Used with bots that require less interations
OffScreenMouseLong()
{
	AntibanOffS2++
	AntibanOffS()
...
;Functional, but dated code
;Needs recode for repo image folder
Logout()
{
	AntibanLog2++
	AntibanLogDis()
...
;Recode needed to include password in the INI file to hide account information
;Recode needed to replace the current imagesearch file locations with a repository folder
LogoutIn()
{
	...
	If (ErrorLevel = 0)
	{
		...
		MouseClickDelay()
		MyClick(2,1)
		;Redacted due to account information being present
		Send, {Enter}
		Loop{
...
;Random mouse movements while clicking on the main screen
AfterClickMove()
{
...
;Random mousemovements which clicking on the minimap
MiniMapAfterClickMove()
{
	Delay := RandomWeight(20, 100, 200)
	Sleep, Delay
...
;Nonfunctional, very dated code, and a concept that should be revised
;Simlates slight mouse difting after a click interaction.
;Example: someone removes their hand from the mouse after clicking on the screen, causing the mouse to move or "drift"
;Known issue: mousemovemt algorithm has issues with small movements and can look unnatural
DriftingMouse()
{
	AntibanCounter++
	AntibanCount()
...
;Functional, dated code, but simple
;Sleeps for a period of time to simulate a person leaving their computer for a breif moment
IdleInGame()
{
	AntibanIdle2++
	AntibanIdle()
...
;Functional and recently recoded
;Opens the skills tab to simulate a human looking at their skill levels
CheckEXP()
{
	AntibanEXP2++
	AntibanEXP()
...
;Used to call function in GUI that randomly assigns a sleep based on total login time
;Redundant code, this should be added to the logout function
LogoutTimer()
{
	LoggedOutFor()
}

;Funtional code, but dated, recode needed for the addition of other tab interactions
AntiTabBrowser()
{
...
;A collection of sleep functions used to mimic human reaction times
;RECODE TO MAKE ALL IN ONE FUNCTION WITH VARIABLE TO SELECT SLEEP
MouseClickDelay()
{
...
;OUTDATED VERSION, SEE CURRENT VERSION BELOW, CODE HAS BEEN SIMPLIFIED FOR READABLITIY / MODULARITY
Dropper()
{
	;MouseAfterClickDelay()
	loop
...
;Needs a recode, current code is dated / nonfunctional
;Used in very niche scenarios
DisconnectedFix()
{
	Loop, 10
	{
...
;OUTDATED VERSION, SEE CURRENT VERSION BELOW, THIS USES AHK DEFAULT PIXELSEARCHING
...
;My our mouse movement function that incorporates 1/3 chance to either undershoot or overshoot targeted location
;Randomly weighted mouse speed with an average of .7
;included pause after mousemovement
;Mimics human-like mousemovement behaviors
...
;Used to check for character idle status by searching for the on screen idle marker color
;Possible recode for GDI+ class, performance on a singular pixel should'nt be noticable
Idlewait()
{
	loop
	{
...
;Verifies that the current mouse location is in proximity of the hovered over color given to a TargetCenter
;Two variables for allowing different colors and variations
...
;Pixelsearches for desired color and calculates a random point within the targets rectangular outline
;Current iteration that incorperates GDI+ which increases performance significantly
...
;Current iteration of my inventory dropping function
DropperV2(){
	...
	Loop{
		LocX := (XChange * XTimes) + (GapX * GapXTimes) + CentX ;Calculate X location from given variables for each iteration of the loop
